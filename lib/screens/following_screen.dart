...
class _FollowingScreenState extends State<FollowingScreen> {
  List<Map<String, dynamic>> _followingStores = [
  ...
  void _toggleFollow(int index) {
    setState(() {
      _followingStores[index]['isFollowing'] = !_followingStores[index]['isFollowing'];
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(_followingStores[index]['isFollowing'] ? 'تمت المتابعة' : 'تم إلغاء المتابعة'), backgroundColor: AppTheme.binanceGreen),
      );
    });
  }
            ...
            children: [
              ClipRRect(
                ...
                child: CachedNetworkImage(
                  imageUrl: _followingStores[index]['image'],
                  width: 60,
                  height: 60,
                  fit: BoxFit.cover,
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(_followingStores[index]['name'], style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                    Text(_followingStores[index]['category'], style: const TextStyle(color: Color(0xFF9CA3AF), fontSize: 12)),
                    Row(
                      children: [
                        SvgPicture.asset('assets/icons/svg/star_gold.svg', width: 12, height: 12),
                        const SizedBox(width: 2),
                        Text('${_followingStores[index]['rating']}', style: const TextStyle(color: Colors.white, fontSize: 11)),
                        const SizedBox(width: 12),
                        Text('${_followingStores[index]['products']} منتج', style: const TextStyle(color: Color(0xFF5E6673), fontSize: 10)),
                      ],
                    ),
                  ],
                ),
              ...
              ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  backgroundColor: _followingStores[index]['isFollowing'] ? AppTheme.binanceCard : AppTheme.binanceGold,
                  foregroundColor: _followingStores[index]['isFollowing'] ? AppTheme.binanceGold : Colors.black,
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
                ),
                child: Text(_followingStores[index]['isFollowing'] ? 'متابع' : 'متابعة'),
              ),
            ],
          ),
        ),
