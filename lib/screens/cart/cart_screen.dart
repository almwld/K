...
class _CartScreenState extends State<CartScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _scaleAnimation = Tween<double>(begin: 0, end: 1).animate(CurvedAnimation(parent: _animationController, curve: Curves.easeOutBack));
  ...
  void _removeItem(String id) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: const Text('تم حذف المنتج من السلة'), backgroundColor: AppTheme.binanceGreen, duration: const Duration(seconds: 1)),
    );
  }
        ...
        title: Row(
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(color: AppTheme.binanceGold, borderRadius: BorderRadius.circular(12)),
              child: Text('${_cartItems.length}', style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 12)),
            ),
          ],
        ),
        centerTitle: true,
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      if (item.discount > 0) ...[
                        Text('${(item.price * (100 - item.discount) ~/ 100).toString()} ريال', style: TextStyle(color: AppTheme.binanceGold, fontWeight: FontWeight.bold, fontSize: 14)),
                        const SizedBox(width: 6),
                        Text('${item.price} ريال', style: const TextStyle(color: Color(0xFF5E6673), decoration: TextDecoration.lineThrough, fontSize: 11)),
                      ] else ...[
                        Text('${item.price} ريال', style: TextStyle(color: AppTheme.binanceGold, fontWeight: FontWeight.bold, fontSize: 14)),
                      ],
                    ],
                  ),
                ],
  ...
  Widget _buildQuantityButton(IconData icon, VoidCallback onTap) {
    return GestureDetector(
      ...
      child: Container(
        width: 28,
        height: 28,
        decoration: BoxDecoration(color: AppTheme.binanceCard, border: Border.all(color: AppTheme.binanceBorder), borderRadius: BorderRadius.circular(8)),
        child: Icon(icon, color: AppTheme.binanceGold, size: 16),
      ),
    );
  }
        ...
        children: [
          ...
          Row(
            children: [
              ...
              ElevatedButton(
                onPressed: () {
                  if (_couponCode.toUpperCase() == 'FLEX10') {
                    setState(() => _couponApplied = true);
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم تطبيق الخصم بنجاح!'), backgroundColor: AppTheme.binanceGreen));
                  } else {
                ...
                style: ElevatedButton.styleFrom(backgroundColor: AppTheme.binanceGold, padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  const Text('متابعة الشراء', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 16)),
                  const SizedBox(width: 8),
                  Text('$_total ريال', style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 16)),
                ],
              ),
            ),
          ),
