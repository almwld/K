...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Widget _buildStoreSummary(String storeId, List<CartItem> items) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [const Icon(Icons.store, color: AppTheme.goldColor, size: 18), const SizedBox(width: 8), Text(items.first.storeName, style: const TextStyle(fontWeight: FontWeight.bold))]),
  ...
  Widget _buildAddressForm(bool isDark) {
    return Container(
      ...
      child: Column(children: [
        TextFormField(controller: _addressController, decoration: InputDecoration(labelText: 'العنوان التفصيلي', prefixIcon: const Icon(Icons.location_on, color: AppTheme.goldColor), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), filled: true, fillColor: isDark ? AppTheme.navyCard : Colors.grey[50]), maxLines: 2, validator: (v) => v?.isEmpty == true ? 'مطلوب' : null),
        const SizedBox(height: 12),
        TextFormField(controller: _phoneController, decoration: InputDecoration(labelText: 'رقم الجوال', prefixIcon: const Icon(Icons.phone, color: AppTheme.goldColor), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), filled: true, fillColor: isDark ? AppTheme.navyCard : Colors.grey[50]), keyboardType: TextInputType.phone),
        const SizedBox(height: 12),
        TextFormField(controller: _notesController, decoration: InputDecoration(labelText: 'ملاحظات (اختياري)', prefixIcon: const Icon(Icons.note, color: AppTheme.goldColor), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), filled: true, fillColor: isDark ? AppTheme.navyCard : Colors.grey[50]), maxLines: 2),
      ]),
    );
  }
        ...
        children: _deliveryOptions.map((option) {
          ...
          return GestureDetector(
            ...
            child: Container(
              ...
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(border: Border.all(color: isSelected ? AppTheme.goldColor : Colors.grey.withOpacity(0.3), width: isSelected ? 2 : 1), borderRadius: BorderRadius.circular(12), color: isSelected ? AppTheme.goldColor.withOpacity(0.1) : Colors.transparent),
              child: Row(children: [Radio(value: option['id'], groupValue: _selectedDelivery, onChanged: (v) => setState(() => _selectedDelivery = v!), activeColor: AppTheme.goldColor), const SizedBox(width: 8), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(option['name'], style: const TextStyle(fontWeight: FontWeight.bold)), Text(option['time'], style: TextStyle(fontSize: 12, color: Colors.grey[600]))])), Text('${option['fee']} ريال', style: const TextStyle(fontWeight: FontWeight.bold))]),
            ),
          );
        }).toList(),
      ),
  ...
  Widget _buildPaymentRadio(String id, String name, IconData icon, {String? subtitle}) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        padding: const EdgeInsets.all(12),
        decoration: BoxDecoration(border: Border.all(color: isSelected ? AppTheme.goldColor : Colors.grey.withOpacity(0.3), width: isSelected ? 2 : 1), borderRadius: BorderRadius.circular(12), color: isSelected ? AppTheme.goldColor.withOpacity(0.1) : Colors.transparent),
        child: Row(children: [Radio(value: id, groupValue: _selectedPayment, onChanged: (v) => setState(() => _selectedPayment = v!), activeColor: AppTheme.goldColor), const SizedBox(width: 8), Icon(icon, color: AppTheme.goldColor), const SizedBox(width: 8), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(name, style: const TextStyle(fontWeight: FontWeight.bold)), if (subtitle != null) Text(subtitle, style: TextStyle(fontSize: 11, color: Colors.grey[600]))]))]),
      ),
    );
  }
  ...
  Widget _buildSummaryRow(String label, String value, {bool isTotal = false}) {
    return Padding(
      ...
      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        ...
        Text(value, style: TextStyle(fontSize: isTotal ? 18 : 14, fontWeight: FontWeight.bold, color: isTotal ? AppTheme.goldColor : null)),
      ]),
    );
  }
    ...
    return Container(
      ...
      child: SafeArea(
        child: ElevatedButton(
          ...
          style: ElevatedButton.styleFrom(backgroundColor: AppTheme.goldColor, padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))),
