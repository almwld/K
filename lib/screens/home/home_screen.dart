...
class _HomeScreenState extends State<HomeScreen> with SingleTickerProviderStateMixin {
  ...
  void _onMarketTabSelected(String tab) {
    setState(() {
      ...
      switch (tab) {
        case 'اكتشف': _displayItems = MarketData.getAllItems().take(25).toList(); break;
        case 'رائج': _displayItems = MarketData.getTrending(); break;
        case 'الاعلانات': _displayItems = MarketData.getOffers(); break;
        default: _displayItems = MarketData.getAllItems().take(25).toList();
      }
    });
  }

  void _onFavoriteToggle(MarketItem item) => setState(() => item.isFavorite = !item.isFavorite);
  ...
  Widget _buildCarouselShimmer() => Container(height: 180, margin: const EdgeInsets.symmetric(horizontal: 16), decoration: BoxDecoration(color: Colors.grey[300], borderRadius: BorderRadius.circular(20)));
  ...
  Widget _buildProductsShimmer() => GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), padding: const EdgeInsets.symmetric(horizontal: 12), gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, childAspectRatio: 0.75), itemCount: 4, itemBuilder: (_, __) => Container(decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16)), child: Column(children: [Container(height: 130, color: Colors.grey[300]), Padding(padding: const EdgeInsets.all(8), child: Column(children: [Container(height: 14, color: Colors.grey[300]), const SizedBox(height: 8), Container(height: 12, width: 80, color: Colors.grey[300])]))])));
