...
class HomeScreen extends StatelessWidget {
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 20),
                  _buildSectionHeader(context, 'متابعاتك', onSeeAll: () {}),
                  ...
                  const SizedBox(height: 20),
                  _buildSectionHeader(context, 'الأسواق الرائجة', onSeeAll: () {}),
                  _buildTrendingMarkets(),
                  const SizedBox(height: 20),
                  _buildSectionHeader(context, 'بالقرب منك', onSeeAll: () {}),
                  _buildNearby(),
        ...
        itemBuilder: (c, i) => GestureDetector(
          ...
          child: Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Row(children: [Container(width: 40, height: 40, decoration: BoxDecoration(color: const Color(0xFFD4AF37).withOpacity(0.1), borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.store, color: Color(0xFFD4AF37))), const SizedBox(width: 8), Expanded(child: Text('متجر ${i+1}', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)))]),
                const SizedBox(height: 8),
                Text('منتج جديد', style: TextStyle(color: const Color(0xFFD4AF37), fontSize: 11)),
                Text('قبل ساعة', style: const TextStyle(color: Color(0xFF5E6673), fontSize: 10)),
              ],
            ),
          ),
        ),
        ...
        itemBuilder: (c, i) => GestureDetector(
          ...
          child: Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Row(children: [Container(width: 35, height: 35, decoration: BoxDecoration(color: const Color(0xFFD4AF37).withOpacity(0.1), borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.store, color: Color(0xFFD4AF37))), const SizedBox(width: 8), Expanded(child: Text('متجر مميز ${i+1}', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold))), Container(width: 6, height: 6, decoration: BoxDecoration(shape: BoxShape.circle, color: i < 3 ? const Color(0xFF0ECB81) : const Color(0xFFF6465D)))]),
                const SizedBox(height: 6),
                Text('إلكترونيات', style: const TextStyle(color: Color(0xFF9CA3AF), fontSize: 10)),
                const SizedBox(height: 4),
                Row(children: [const Icon(Icons.star, color: Colors.amber, size: 12), const SizedBox(width: 2), Text('${4.5 + i*0.1}', style: const TextStyle(color: Colors.white, fontSize: 11)), const Spacer(), Text(i < 3 ? 'مفتوح' : 'مغلق', style: TextStyle(color: i < 3 ? const Color(0xFF0ECB81) : const Color(0xFFF6465D), fontSize: 10))]),
              ],
            ),
          ),
        ),
        ...
        itemBuilder: (c, i) => GestureDetector(
          ...
          child: Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Container(height: 100, decoration: BoxDecoration(color: const Color(0xFFF6465D).withOpacity(0.2), borderRadius: const BorderRadius.vertical(top: Radius.circular(12))), child: Center(child: Text('-${offers[i]['discount']}%', style: const TextStyle(color: Color(0xFFF6465D), fontSize: 24, fontWeight: FontWeight.bold)))),
                Padding(
                  padding: const EdgeInsets.all(8),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(offers[i]['name']!, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 4),
                      Row(children: [Text(offers[i]['price']!, style: const TextStyle(color: Color(0xFFD4AF37), fontWeight: FontWeight.bold)), const SizedBox(width: 4), Text(offers[i]['old']!, style: const TextStyle(color: Color(0xFF5E6673), decoration: TextDecoration.lineThrough, fontSize: 10))]),
                    ],
                  ),
                ),
              ],
      ...
      children: markets.map((m) => Container(
        ...
        child: Row(
          children: [
            ...
            Expanded(child: Text(m['volume']!, style: const TextStyle(color: Color(0xFF9CA3AF)), textAlign: TextAlign.end)),
            const SizedBox(width: 12),
            Expanded(child: Text('${m['items']}', style: const TextStyle(color: Color(0xFF5E6673)), textAlign: TextAlign.end)),
          ],
        ),
      )).toList(),
    );
  ...
  Widget _buildNearby() {
    final nearby = [
      {'name': 'متجر الذهبية', 'distance': '0.3', 'rating': 4.5},
      {'name': 'متجر الكس', 'distance': '0.8', 'rating': 4.8},
      {'name': 'متجر السيم', 'distance': '1.2', 'rating': 4.3},
      {'name': 'مطعم النور', 'distance': '0.5', 'rating': 4.6},
    ];
    return GridView.count(
      ...
      crossAxisSpacing: 12,
      mainAxisSpacing: 12,
      children: nearby.map((n) => Container(
        padding: const EdgeInsets.all(12),
        decoration: BoxDecoration(color: const Color(0xFF1E2329), borderRadius: BorderRadius.circular(12)),
        child: Row(
          children: [
            Container(width: 40, height: 40, decoration: BoxDecoration(color: const Color(0xFFD4AF37).withOpacity(0.1), borderRadius: BorderRadius.circular(10)), child: const Icon(Icons.store, color: Color(0xFFD4AF37))),
            const SizedBox(width: 12),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                ...
                children: [
                  ...
                  Row(children: [Text('${n['distance']} كم', style: const TextStyle(color: Color(0xFF9CA3AF), fontSize: 10)), const SizedBox(width: 8), const Icon(Icons.star, size: 10, color: Colors.amber), Text('${n['rating']}', style: const TextStyle(color: Color(0xFF9CA3AF), fontSize: 10))]),
                ],
              ),
            ),
          ],
