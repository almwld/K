...
class _HomeScreenState extends State<HomeScreen> {
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        slivers: [
          ...
          // عروض حصرية
          SliverToBoxAdapter(child: _buildExclusiveOffers()),
          const SliverToBoxAdapter(child: SizedBox(height: 16)),

          // إعلانات مميزة
          SliverToBoxAdapter(child: _buildFeaturedAds()),
          const SliverToBoxAdapter(child: SizedBox(height: 16)),
          ...
          // عروض اليوم
          SliverToBoxAdapter(child: _buildDailyDeals()),
          const SliverToBoxAdapter(child: SizedBox(height: 16)),
  ...
  Widget _buildExclusiveOffers() {
    final offers = [
      {'title': 'خصم 50%', 'store': 'متجر التقنية', 'endDate': 'ينتهي خلال 3 أيام', 'image': _electronicsImage},
      {'title': 'خصم 30%', 'store': 'أزياء فلكس', 'endDate': 'ينتهي خلال يومين', 'image': _fashionImage},
      {'title': 'خصم 40%', 'store': 'مطعم فلكس', 'endDate': 'ينتهي اليوم', 'image': _foodImage},
    ];
    
    return Column(
      children: [
        _buildSectionHeader('عروض حصرية'),
        SizedBox(
          height: 200,
          child: ListView.builder(
            scrollDirection: Axis.horizontal,
                      ...
                      child: Column(
                        ...
                        children: [
                          Container(
                            ...
                            child: Text(offer['title'] as String, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontFamily: 'Changa')),
                          ),
                          const SizedBox(height: 8),
                          Text(offer['store'] as String, style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold, fontFamily: 'Changa')),
                          const SizedBox(height: 4),
                          Row(
                            children: [
                              const Icon(Icons.timer, color: Color(0xFFF0B90B), size: 14),
                              const SizedBox(width: 4),
                              Text(offer['endDate'] as String, style: const TextStyle(color: Colors.white70, fontSize: 12, fontFamily: 'Changa')),
                            ],
                          ),
                        ],
                      ),
  ...
  Widget _buildFeaturedAds() {
    final ads = [
      {'title': 'أفضل العروض', 'subtitle': 'اكتشف أحدث المنتجات', 'color': const Color(0xFFF0B90B)},
      {'title': 'توصيل مجاني', 'subtitle': 'للطلبات فوق 500 ريال', 'color': const Color(0xFF0ECB81)},
    ];
    
    return Column(
      children: [
        _buildSectionHeader('إعلانات مميزة'),
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 16),
          child: Row(
            children: ads.map((ad) {
              return Expanded(
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: LinearGradient(
                      colors: [ad['color'] as Color, (ad['color'] as Color).withOpacity(0.7)],
                      begin: Alignment.topLeft,
                      end: Alignment.bottomRight,
                    ),
                    borderRadius: BorderRadius.circular(16),
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(ad['title'] as String, style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold, fontFamily: 'Changa')),
                      const SizedBox(height: 4),
                      Text(ad['subtitle'] as String, style: const TextStyle(color: Colors.white70, fontSize: 12, fontFamily: 'Changa')),
                      const SizedBox(height: 12),
                      Container(
                        ...
                        child: const Text('اكتشف', style: TextStyle(color: Colors.black, fontSize: 12, fontWeight: FontWeight.bold, fontFamily: 'Changa')),
                      ),
                    ],
                  ),
                ),
  ...
  Widget _buildCategories() {
    final categories = [
      {'name': 'إلكترونيات', 'icon': Icons.devices, 'color': const Color(0xFF2196F3)},
      {'name': 'أزياء', 'icon': Icons.checkroom, 'color': const Color(0xFFE91E63)},
      {'name': 'سيارات', 'icon': Icons.directions_car, 'color': const Color(0xFFF6465D)},
      {'name': 'عقارات', 'icon': Icons.home, 'color': const Color(0xFF4CAF50)},
      {'name': 'أثاث', 'icon': Icons.chair, 'color': const Color(0xFFFF9800)},
      {'name': 'مطاعم', 'icon': Icons.restaurant, 'color': const Color(0xFF9C27B0)},
      {'name': 'رياضة', 'icon': Icons.sports_soccer, 'color': const Color(0xFF00BCD4)},
      {'name': 'كتب', 'icon': Icons.menu_book, 'color': const Color(0xFF795548)},
    ];
    
    return Column(
      children: [
        _buildSectionHeader('تصفح الفئات'),
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 16),
          child: GridView.builder(
            shrinkWrap: true,
            ...
            itemBuilder: (context, index) {
              ...
              return Column(
                children: [
                  ...
                  const SizedBox(height: 6),
                  Text(cat['name'] as String, style: const TextStyle(color: Colors.white, fontSize: 11, fontFamily: 'Changa'), textAlign: TextAlign.center),
                ],
              );
            },
          ),
  ...
  Widget _buildBestSellers() {
    final products = sampleProducts.take(6).toList();
                        ...
                        children: [
                          ...
                          Positioned(
                            ...
                            child: Container(
                              ...
                              child: const Text('🔥 مبيع', style: TextStyle(color: Colors.black, fontSize: 9, fontWeight: FontWeight.bold, fontFamily: 'Changa')),
                            ),
                          ),
                        ],
                      ),
                      Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(product.title, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w500, fontSize: 12, fontFamily: 'Changa'), maxLines: 1),
                            const SizedBox(height: 4),
                            Text(product.formattedPrice, style: TextStyle(color: const Color(0xFFF0B90B), fontWeight: FontWeight.bold, fontSize: 14, fontFamily: 'Changa')),
                            const SizedBox(height: 4),
                            Row(
                              children: [
                                const Icon(Icons.star, color: Colors.amber, size: 12),
                                ...
                                Text('${product.rating}', style: const TextStyle(color: Color(0xFF9CA3AF), fontSize: 10, fontFamily: 'Changa')),
                                const Spacer(),
                                Text('${product.reviewCount}+', style: const TextStyle(color: Color(0xFF5E6673), fontSize: 9, fontFamily: 'Changa')),
                              ],
                            ),
                          ],
                        ),
  ...
  Widget _buildDailyDeals() {
    final deals = [
      {'name': 'سماعات لاسلكية', 'price': '199', 'oldPrice': '399', 'sold': 85, 'total': 100, 'image': _electronicsImage},
      {'name': 'ساعة ذكية', 'price': '299', 'oldPrice': '599', 'sold': 45, 'total': 80, 'image': _fashionImage},
    ];
    
    return Column(
      children: [
        _buildSectionHeader('عروض اليوم', showTimer: true),
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 16),
          child: Column(
              ...
              return Container(
                ...
                child: Row(
                  children: [
                    ClipRRect(
                      borderRadius: BorderRadius.circular(10),
                      child: _buildCachedImage(deal['image'] as String, width: 70, height: 70),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(deal['name'] as String, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontFamily: 'Changa')),
                          const SizedBox(height: 4),
                          Row(
                            children: [
                              Text('${deal['price']} ريال', style: TextStyle(color: const Color(0xFFF0B90B), fontWeight: FontWeight.bold, fontFamily: 'Changa')),
                              const SizedBox(width: 8),
                              Text('${deal['oldPrice']} ريال', style: const TextStyle(color: Color(0xFF5E6673), decoration: TextDecoration.lineThrough, fontSize: 11, fontFamily: 'Changa')),
                            ],
                          ),
                          const SizedBox(height: 8),
                          ClipRRect(
                            ...
                            child: LinearProgressIndicator(
                              value: (deal['sold'] as int) / (deal['total'] as int),
                              backgroundColor: const Color(0xFF2B3139),
                              valueColor: const AlwaysStoppedAnimation<Color>(Color(0xFFF6465D)),
                              minHeight: 6,
                            ),
                          ),
                          const SizedBox(height: 4),
                          Text('تم بيع ${deal['sold']} من ${deal['total']}', style: const TextStyle(color: Color(0xFF9CA3AF), fontSize: 10, fontFamily: 'Changa')),
                        ],
                      ),
                    ),
                  ],
  ...
  Widget _buildBrands() {
    final brands = [
      {'name': 'Apple', 'logo': _electronicsImage},
      {'name': 'Samsung', 'logo': _electronicsImage},
      {'name': 'Nike', 'logo': _fashionImage},
      {'name': 'Adidas', 'logo': _fashionImage},
      {'name': 'Sony', 'logo': _electronicsImage},
      {'name': 'LG', 'logo': _electronicsImage},
    ];
    
    return Column(
      children: [
        _buildSectionHeader('ماركات عالمية'),
        SizedBox(
          height: 90,
          child: ListView.builder(
            scrollDirection: Axis.horizontal,
                ...
                child: Column(
                  ...
                  children: [
                    ClipRRect(
                      ...
                      child: _buildCachedImage(brand['logo'] as String, width: 45, height: 45),
                    ),
                    const SizedBox(height: 4),
                    Text(brand['name'] as String, style: const TextStyle(color: Colors.white, fontSize: 11, fontFamily: 'Changa')),
                  ],
                ),
              );
            },
  ...
  Widget _buildArticles() {
    final articles = [
      {'title': 'كيف تختار هاتفك الذكي؟', 'readTime': '5 دقائق', 'image': _electronicsImage},
      {'title': 'أفضل 10 سيارات 2026', 'readTime': '8 دقائق', 'image': _carsImage},
      {'title': 'نصائح لتوفير المال', 'readTime': '3 دقائق', 'image': _realEstateImage},
    ];
    
    return Column(
      children: [
        _buildSectionHeader('مقالات ونصائح'),
        SizedBox(
          height: 160,
          child: ListView.builder(
            scrollDirection: Axis.horizontal,
                  ...
                  children: [
                    ...
                    Positioned(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(article['title'] as String, style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.bold, fontFamily: 'Changa')),
                          const SizedBox(height: 4),
                          Row(
                            children: [
                              const Icon(Icons.access_time, color: Color(0xFFF0B90B), size: 12),
                              const SizedBox(width: 4),
                              Text(article['readTime'] as String, style: const TextStyle(color: Colors.white70, fontSize: 11, fontFamily: 'Changa')),
                            ],
                          ),
                        ],
                      ),
  ...
  // تحديث دالة _buildSectionHeader لدعم المؤقت
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: 4),
                      Text('12:45:30', style: TextStyle(color: const Color(0xFFF6465D), fontSize: 12, fontWeight: FontWeight.bold, fontFamily: 'Changa')),
                    ],
                  ),
                ),
              ],
