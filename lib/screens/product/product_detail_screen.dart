...
class _ProductDetailScreenState extends State<ProductDetailScreen> {
  ...
  final Map<String, dynamic> _product = {
    ...
    'rating': 4.8,
    'reviews': 128,
    'description': 'iPhone 15 Pro بشاشة 6.1 بوصة، كاميرا 48 ميجابكسل، شريحة A17 Pro، بطارية تدوم طوال اليوم',
    'seller': 'متجر التقنية',
    'sellerRating': 4.9,
    'sellerReviews': 1234,
    'inStock': true,
  ...
  final List<Map<String, dynamic>> _reviews = [
    {'name': 'أحمد محمد', 'rating': 5, 'comment': 'منتج رائع جداً، أنصح به', 'date': '2024-04-20', 'avatar': 'https://randomuser.me/api/portraits/men/1.jpg'},
    {'name': 'سارة علي', 'rating': 4, 'comment': 'جيد ولكن السعر مرتفع قليلاً', 'date': '2024-04-18', 'avatar': 'https://randomuser.me/api/portraits/women/1.jpg'},
  ];
  ...
  final List<Map<String, dynamic>> _relatedProducts = [
    ...
    {'id': '4', 'name': 'سماعات ايربودز', 'price': 35000, 'image': 'https://images.unsplash.com/photo-1605464315542-bda3e2f4e605?w=200', 'rating': 4.7},
  ];

  void _addToCart() {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('تمت إضافة ${_product['name']} إلى السلة'), backgroundColor: AppTheme.binanceGreen),
    );
  }
  ...
  Widget _buildAppBar() {
    return SliverAppBar(
      ...
      pinned: true,
      backgroundColor: AppTheme.binanceDark,
      flexibleSpace: FlexibleSpaceBar(background: Container(color: AppTheme.binanceDark)),
  ...
  Widget _buildProductImages() {
    return Column(children: [
      ...
      Row(mainAxisAlignment: MainAxisAlignment.center, children: List.generate((_product['images'] as List).length, (i) => Container(width: 8, height: 8, margin: const EdgeInsets.symmetric(horizontal: 4), decoration: BoxDecoration(shape: BoxShape.circle, color: _currentImageIndex == i ? AppTheme.binanceGold : AppTheme.binanceBorder)))),
    ]);
  }
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ...
          if (_product['oldPrice'] != null) ...[const SizedBox(width: 8), Text('${_product['oldPrice']} ريال', style: const TextStyle(color: Color(0xFF5E6673), decoration: TextDecoration.lineThrough, fontSize: 16))],
        ]),
        const SizedBox(height: 16),
        Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: AppTheme.binanceCard, borderRadius: BorderRadius.circular(12)), child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [const Icon(Icons.star, size: 12, color: Colors.amber), const SizedBox(width: 2), Text('${_product['sellerRating']}', style: const TextStyle(color: Colors.white)), const SizedBox(width: 4), Text('(${_product['sellerReviews']})', style: const TextStyle(color: Color(0xFF9CA3AF)))])),
          ])),
        ...
        Row(children: [
          ...
          Container(decoration: BoxDecoration(border: Border.all(color: AppTheme.binanceBorder), borderRadius: BorderRadius.circular(8)), child: Row(children: [
  ...
  Widget _buildSpecsTab() {
    return ListView.builder(
      ...
      itemBuilder: (context, index) => Padding(
        ...
        child: Row(children: [const Icon(Icons.check_circle, color: AppTheme.binanceGold, size: 16), const SizedBox(width: 8), Text((_product['specs'] as List)[index], style: const TextStyle(color: Colors.white))]),
      ),
    );
  }
  ...
  Widget _buildRelatedProducts() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 12),
      SizedBox(height: 200, child: ListView.builder(scrollDirection: Axis.horizontal, padding: const EdgeInsets.symmetric(horizontal: 16), itemCount: _relatedProducts.length, itemBuilder: (_, i) => GestureDetector(
        ...
        child: Container(width: 150, margin: const EdgeInsets.only(right: 12), decoration: BoxDecoration(color: AppTheme.binanceCard, borderRadius: BorderRadius.circular(12), border: Border.all(color: AppTheme.binanceBorder)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
