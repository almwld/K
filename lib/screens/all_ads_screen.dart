...
class _AllAdsScreenState extends State<AllAdsScreen> {
              ...
              child: Column(
                children: [
                  DropdownButtonFormField<String>(
                    ...
                    decoration: InputDecoration(labelText: 'الفئة', filled: true, fillColor: AppTheme.getCardColor(context), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none)),
                    items: _categories.map((c) => DropdownMenuItem(value: c, child: Text(c))).toList(),
                    onChanged: (v) { setState(() => _selectedCategory = v!); _filterProducts(); },
                  ),
                  const SizedBox(height: 12),
                  DropdownButtonFormField<String>(
                    value: _selectedCity,
                    decoration: InputDecoration(labelText: 'المدينة', filled: true, fillColor: AppTheme.getCardColor(context), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none)),
                    items: _cities.map((c) => DropdownMenuItem(value: c, child: Text(c))).toList(),
                    onChanged: (v) { setState(() => _selectedCity = v!); _filterProducts(); },
                  ),
                  const SizedBox(height: 12),
                  Row(children: [
                    Expanded(child: Text('السعر: ${_priceRange.start.toStringAsFixed(0)} - ${_priceRange.end.toStringAsFixed(0)} ر.ي', style: const TextStyle(fontSize: 12))),
                    TextButton(onPressed: () => setState(() => _priceRange = const RangeValues(0, 10000000)), child: const Text('إعادة')),
                  ]),
                  RangeSlider(values: _priceRange, min: 0, max: 10000000, divisions: 100, activeColor: AppTheme.goldColor, onChanged: (values) { setState(() => _priceRange = values); _filterProducts(); }),
                  const SizedBox(height: 12),
                  Row(children: [
                    const Text('الحد الأدنى للتقييم: ', style: TextStyle(fontSize: 12)),
                    Row(children: List.generate(5, (index) {
                      return IconButton(icon: Icon(index < _minRating ? Icons.star : Icons.star_border, color: Colors.amber, size: 20), onPressed: () { setState(() => _minRating = index + 1.0); _filterProducts(); }, padding: EdgeInsets.zero, constraints: const BoxConstraints());
                    })),
                  ]),
                  const SizedBox(height: 12),
                  SizedBox(width: double.infinity, child: OutlinedButton.icon(onPressed: _resetFilters, icon: const Icon(Icons.refresh), label: const Text('إعادة تعيين جميع الفلاتر'), style: OutlinedButton.styleFrom(side: const BorderSide(color: AppTheme.goldColor)))),
          ...
          Expanded(
            child: _isLoading
                ...
                : _filteredProducts.isEmpty
                    ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.inbox, size: 80, color: AppTheme.goldColor.withOpacity(0.5)), const SizedBox(height: 16), const Text('لا توجد إعلانات')]))
                    : GridView.builder(
                        padding: const EdgeInsets.all(16),
                                    ...
                                    child: Padding(
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          Row(children: [const Icon(Icons.location_on, size: 10, color: Colors.grey), const SizedBox(width: 2), Expanded(child: Text(product.city, style: const TextStyle(fontSize: 10, color: Colors.grey), maxLines: 1))]),
  ...
  Widget _buildSortChip(String label, String value) {
    ...
    return FilterChip(
      label: Text(label),
      selected: isSelected,
      onSelected: (selected) { setState(() => _sortBy = value); _sortProducts(); },
...
// تحديث الأسعار لتكون منطقية
// آيفون 450,000 ريال ✅
// ماك بوك 1,800,000 ريال ✅
// سامسونج 380,000 ريال ✅
